...
const Saved: React.FC = () => {
  ...
  const handleRemove = async (productId: string) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/user/save-later/remove`, {
        method: "POST",
        headers: { 
        ...
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
            <div className="relative mb-6">
              <div className="h-32 w-32 rounded-full bg-gradient-to-br from-amber-100 to-orange-100 dark:from-amber-900/20 dark:to-orange-900/20 flex items-center justify-center">
                <BookmarkX className="h-16 w-16 text-amber-400 dark:text-amber-500" />
              </div>
              <Clock className="absolute -top-2 -right-2 h-8 w-8 text-blue-400 animate-pulse" />
            </div>
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
              No saved items yet
            </h2>
            <p className="text-gray-600 dark:text-gray-400 mb-8 max-w-md">
              Save products you want to revisit later. They'll be stored here so you can easily find them when you're ready to purchase!
            </p>
            <Button
              onClick={() => navigate("/")}
